defmodule Tutorials.Utils.Error do
  @derive {Jason.Encoder, only: [:code, :reason]}
  defexception [:code, :reason, :message]

  # %__MODULE__: refers to the name of the module it is used in.
  # For example, if you name your module MyModule,
  # then %__MODULE_{some_field: value} will be considered %MyModule{some_field: value} in the module MyModule.

  @type error_code :: String.t() # Defines a new data type named error_code
  @type t :: %__MODULE__{code: error_code, reason: String.t(), message: String.t()} # Defines a new data type named t. This data type is a struct with three fields
  ...
  def c_UNAUTHENTICATED, do: "unauthenticated"
  def c_FORBIDDEN, do: "forbidden"
  def c_BAD_REQUEST, do: "bad_request"

  @impl true # is defined as an implementation function for the Jason.Encoder protocol. Checks that the message/1 function complies with the requirements of the Jason.Encoding protocol
  ...
  def exception(opts) do
    code = Keyword.get(opts, :code, c_INTERNAL_SERVER_ERROR()) # If :code is not provided in opts, by default it will use the error code c_INTERNAL_SERVER_ERROR()
    reason = Keyword.get(opts, :reason)
    msg = Keyword.get(opts, :message)
    %__MODULE__{code: code, reason: reason, message: msg}
  end

  defp _convert_to_atom(code) when is_binary(code) do
    String.to_existing_atom(code)
  end

  defp _convert_code(code) when is_binary(code) do
    _convert_to_atom(code)
  end

  defp _convert_code(code) do
    code
  end
  ...
  def transform(%__MODULE__{code: code} = reason) do
    ...
    case code do
      ...
      :bad_request  -> {:bad_request, reason}
  ...
  def transform(reason) do
    ...
    case reason do
      %Ecto.InvalidChangesetError{} ->
        {:bad_request, _transform_changeset_errors(reason.changeset)}
